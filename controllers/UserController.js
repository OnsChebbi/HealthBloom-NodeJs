...
exports.login = async (req, res) => {
  try {
    ...
    } else res.status(402).send("email is wrong");
  } catch (err) {
    res.status(400).send(err);
  }
};
  ...
  User.findByIdAndUpdate({ _id: id }, user)
    .then((result) => {
      if (user.Role === "Patient") {
        ...
        patient.save().then(() => {
          user._patient = patient._id;
          User.findByIdAndUpdate({ _id: id }, user).then((resultt) => {
  ...
  PasswordReset.deleteMany({ userId: _id })
    .then((result) => {
      ...
      bcrypt.hash(resetString, 10).then((hashedResetString) => {
        ...
        newPwdRest.save().then(() => {
          transporter.sendMail(mailParams).then(
            res.json({
              status: "Pending",
  ...
  await PasswordReset.find({ userId: userId }, (err, result) => {
    ...
    if (result.length > 0) {
      if (result[0].expiredAt < Date.now()) {
        ...
        bcrypt.compare(resetString, result[0].resetString).then((result) => {
          if (result) {
